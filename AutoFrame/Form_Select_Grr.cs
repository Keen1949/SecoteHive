...
namespace AutoFrame
{
    public partial class Form_Select_Grr : Form
    {
        ...
        private void InitComboBox()
        {
            ...
            dataGridView_GRR.Controls.Add(m_comboBoxStation);
            dataGridView_GRR.Controls.Add(m_comboBoxMethod);
        ...
        private void SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            dataGridView_GRR.CurrentCell.Value = combo.Text;
        }

        private void InitDataGridView()
        {
            dataGridView_GRR.Columns.Clear();
            dataGridView_GRR.RowsDefaultCellStyle.BackColor = Color.Ivory;
            dataGridView_GRR.AlternatingRowsDefaultCellStyle.BackColor = Color.PaleTurquoise;

            string[] Heads = { "名称", "站位", "方法" };

            foreach(string head in Heads)
            {
                int col = dataGridView_GRR.Columns.Add(head,head);
                dataGridView_GRR.Columns[col].SortMode = DataGridViewColumnSortMode.NotSortable;
                dataGridView_GRR.Columns[col].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            }
        }

        private void Form_Select_Calib_Load(object sender, EventArgs e)
        {
            SystemMgrEx.GetInstance().UpdateGridFromParam(RunMode.GRR, dataGridView_GRR);
            UpdateRadioButtons();
        ...
        {
            panel_Grr.Controls.Clear();
            m_listRadioButton.Clear();

            int btnWidth = 200;
            int btnHeight = 36;

            int cols = panel_Grr.Width / btnWidth;
            if (cols < 1)
            {
                cols = 1;
            }
            ...
            //动态创建按钮
            int nIndex = 0;
            foreach(var item in SystemMgrEx.GetInstance().m_dictGrrs.Values)
            {
                ...
                if (SystemMgrEx.GetInstance().CurrentGrr == item.m_strName)
                {
                    radio.Checked = true;
                }
                else
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            foreach (var radio in m_listRadioButton)
            {
                ...
                {       
                    ...
                    if (SystemMgrEx.GetInstance().m_dictGrrs.TryGetValue(radio.Text,out info))
                    {
                        SystemMgrEx.GetInstance().CurrentGrr = radio.Text;
                    ...
                    else
                    {
                        ...
                        {
                            MessageBox.Show("This test item does not exist", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);

                        }
                        else
                        {
                            MessageBox.Show("不存在此测试项", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void dataGridView_Grr_CurrentCellChanged(object sender, EventArgs e)
        {
            m_comboBoxStation.Visible = false;
            m_comboBoxMethod.Visible = false;

            if (dataGridView_GRR.CurrentCell == null)
            {
                return;
            }

            int rowIndex = dataGridView_GRR.CurrentCell.RowIndex;
            int colIndex = dataGridView_GRR.CurrentCell.ColumnIndex;

            if (colIndex == 1)
            {
                Rectangle rect = dataGridView_GRR.GetCellDisplayRectangle(colIndex, rowIndex, true);
                ...
                if (dataGridView_GRR.CurrentCell.Value != null)
                {
                    m_comboBoxStation.Text = dataGridView_GRR.CurrentCell.Value.ToString();
                }
                else
                {
            ...
            {
                if (dataGridView_GRR.Rows[rowIndex].Cells[1].Value == null)
                {
                    return;
                }

                UpdateComboBoxMethod(dataGridView_GRR.Rows[rowIndex].Cells[1].Value.ToString());
                ...
                if (dataGridView_GRR.CurrentCell.Value != null)
                {
                    m_comboBoxMethod.Text = dataGridView_GRR.CurrentCell.Value.ToString();
                }
                else
                {
        ...
        private void dataGridView_Grr_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
            m_comboBoxStation.Visible = false;
            m_comboBoxMethod.Visible = false;
        }

        private void dataGridView_Grr_Scroll(object sender, ScrollEventArgs e)
        {
            m_comboBoxStation.Visible = false;
            m_comboBoxMethod.Visible = false;
        }
        ...
        {
            int nSelectRow = dataGridView_GRR.CurrentCell.RowIndex;

            if (nSelectRow < dataGridView_GRR.Rows.Count - 1)
            {
                dataGridView_GRR.Rows.RemoveAt(nSelectRow);
            }
        }

        private void button_Apply_Click(object sender, EventArgs e)
        {
            SystemMgrEx.GetInstance().UpdateParamFromGrid(RunMode.GRR, dataGridView_GRR);
