...
namespace Plc
{
    ...
    public class Plc_ModbusTcp : PlcDevice
    {
        ...
        public Plc_ModbusTcp(int nIndex, string strName, TcpLink tl, PlcAddressMap mp) : base(nIndex, strName, mp)
        {
            m_TcpLink = tl;
            m_bOpen = false;

            m_Modbus = new ModbusTcp(tl);

            // 每一款PLC的限制不一样，此处的设置根据KEYENCE设置
            m_CmdLimit = new PlcCmdCountLimit(2000, 1968, 125, 123);
        }
        ...
        /// <param name="mp">元件地址映射表</param>
        public Plc_ModbusTcp(int nIndex, string strName, TcpLink tl, int nId,PlcAddressMap mp) : base(nIndex, strName, mp)
        {
            m_TcpLink = tl;
            m_bOpen = false;

            m_Modbus = new ModbusTcp(tl,nId);
